...
Sampling, stop sequences, streaming output, and a local OpenAI-compatible server are done. See
[Parity with the reference](#parity-with-the-reference) for what is not.
...
| RAM | 16 GB minimum, 24 GB+ recommended. Context and the expert-slot pool auto-size from installed RAM: 2048 tokens / 16 slots per layer at ≤16 GB, 4096 tokens / 24–32 slots above. |
...
Unknown arguments are a hard error (exit 2), never silently ignored.
...
| `--context <n>` | Max context in tokens; 0 auto-sizes from RAM. Alias: `--max-context` |
...
Measured on a 24 GB Ryzen/Radeon desktop box, 24-token generation.

| | Turbo-WinFare | TurboFieldfare (reference) |
|---|---|---|
| Decode | **8.8–9.0 tok/s** (24 GB, auto slots) | 5.1–6.3 tok/s on an 8 GB M2 Air |
| Decode, constrained | **6.54 tok/s** at 16 slots (the 16 GB Legion Go S target) | — |
| Prefill | ~5.7 tok/s | ~28 tok/s |

Where decode time goes now: ~34% expert I/O, ~41% GPU wait, ~9% LM head, ~16% other CPU. The
engine is nowhere near compute- or bandwidth-bound, so the remaining headroom is in kernel
efficiency and batched prefill, not in hardware. [docs/PERFORMANCE.md](docs/PERFORMANCE.md) has
the full optimization history, including the changes that were measured and *rejected*.

> **Benchmarking warning.** Run-to-run drift on this class of machine is larger than most single
> optimizations — the same binary measured 8.6–9.0 tok/s in one session and 7.1–7.9 in another.
> Compare variants **interleaved within one session**, never across sessions.
